...
function sum(a, b) {    //  매개변수 a, b를 받아서
    return a + b;       //  결과를 반환한다 -> return 문을 만나면 함수 종료
}
...
//  익명 함수의 활용 1. 즉시 실행 함수 -> 주로 페이지의 초기화를 할때 유용함
let initText;
(function(number){
...
// console.log("textList : ", textList);   //  textList 는 함수 내부에 있어서 외부에서 참조 불가능 : Error
...
function getNumberTotal() { //  매개변수 선언하지 않음
    ...
    //  전달받은 모든 매개변수를 합산해서 출력
    let result = 0;
    for (let i = 0; i < arguments.length; i++){
        //  전달받은 매개변수가 산술연산 가능한가 ?
...
//  JavaScript 의 함수는 객체 : 변수에 참조되거나 다른 함수의 매개변수로 전달될 수 있다
